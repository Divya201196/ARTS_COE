...
class ExamTimetableController extends Controller
{
 ...
 /* public function actionExaminerReport()
    {
        $model = new AbsentEntry();
        $ans = new AnswerPacket();
        $examTimetable = new ExamTimetable();
        ...
        //Yii::$app->ShowFlashMessages->setMsg('Welcome','Welcome to Consolidate '.ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_ABSENT).' List');
        return $this->render('examiner-report', [
            'model' => $model,
            'examTimetable' => $examTimetable,
            'ans'=>$ans,
    ...
    public function actionExaminerReport()
    {
        ...
        if(Yii::$app->request->post())
        {
            ...
            $exam_date = $_POST['valuation_date'];
           
            $board = $_POST['ValuationFacultyAllocate']['board'];
           // print_r($board);exit;
            $det_cat_type = Yii::$app->db->createCommand("select coe_category_type_id from coe_category_type where category_type like '%detain%'")->queryScalar();
            ...
            $det_disc_type = Yii::$app->db->createCommand("select coe_category_type_id from coe_category_type where category_type like '%Discontinued%'")->queryScalar();
             ...
             $boardname = Categorytype::findOne($board);
         // print_r($boardname);exit;  
            $faculty = Yii::$app->db->createCommand("SELECT DISTINCT(A.faculty_name),A.college_code,faculty_experience,phone_no,email FROM coe_valuation_faculty A JOIN coe_valuation_faculty_allocate B ON A.coe_val_faculty_id=B.coe_val_faculty_id JOIN coe_category_type C on C.coe_category_type_id=B.exam_month JOIN coe_category_type D on D.category_type=B.board WHERE B.exam_month='" . $exam_month . "' AND B.exam_year='" . $exam_year . "'  AND B.valuation_date='".date('Y-m-d',strtotime($exam_date))."' AND B.board='" . $boardname['category_type'] . "'  ORDER BY valuation_date")->queryAll();
        //print_r($faculty);exit;
            ...
            {
                Yii::$app->ShowFlashMessages->setMsg('Error','NO DATA FOUND');
                return $this->redirect(['exam-timetable/examiner-report']);
            }
        }
        else
        {
 ...
 public function actionExaminerReportExcel()
    {    
    ...
    public function actionExaminerReportPdf()
    {
        require(Yii::getAlias('@webroot/includes/use_institute_info.php')); 
          $content = $_SESSION['student_count_repo'];
        $pdf = new Pdf([
     ...
     public function actionValuationfacultyreport()
    {
        $model = new MarkEntry();
        $markentrymaster = new MarkEntryMaster();
        $galley = new HallAllocate();
        $factallModel = new ValuationFacultyAllocate();
        Yii::$app->ShowFlashMessages->setMsg('Welcome', 'Welcome to Faculty Information');
        return $this->render('valuationfacultyreport', [
            'model' => $model, 'galley' => $galley,
            'factallModel' =>$factallModel,
        ]);
    }
    public function actionValuationfacultyreportdata()
    {
        ...
        $board1 = Yii::$app->request->post('board'); 
        ...
        $sn = 1;
        $query = new Query();
        $query->select('distinct (A.faculty_name) as faculty_name,A.college_code,A.faculty_experience,A.phone_no,A.email')
            ->from('coe_valuation_faculty A')
            ->join('JOIN', 'coe_valuation_faculty_allocate B', 'A.coe_val_faculty_id=B.coe_val_faculty_id')
            ->join('JOIN', 'coe_category_type C', 'C.coe_category_type_id=B.exam_month')
            ->join('JOIN', 'coe_category_type D', 'D.category_type=B.board')
            ->where(['B.exam_year'=>$year,'B.exam_month'=>$month,'B.board'=>$board]);
        $faculty = $query->createCommand()->queryAll();

        //print_r($faculty);exit;
       /* $faculty = Yii::$app->db->createCommand("SELECT DISTINCT(A.faculty_name),A.college_code,faculty_experience,phone_no,email FROM coe_valuation_faculty A JOIN coe_valuation_faculty_allocate B ON A.coe_val_faculty_id=B.coe_val_faculty_id JOIN coe_category_type C on C.coe_category_type_id=B.exam_month JOIN coe_category_type D on D.category_type=B.board WHERE B.exam_month='" . $month . "' AND B.exam_year='" . $year . "' ORDER BY month")->queryAll();*/
       ...
       // print_r($faculty);exit;
        ...
        if (count($faculty) > 0) {
            ...
            $table .= '
                        ...
                        <tr>
                        <td colspan=18><b>'.$year.'-'.strtoupper($month_name).'  - FACULTY INFORMATION </b> </td>
                        </tr>';
            ...
            foreach ($faculty as $faculty1) {
                $table .= '
                    <tr>
                        <td> ' . $sn . ' </td>
                        <td> ' . $faculty1['faculty_name'] . ' </td>
                        <td> ' . $faculty1['college_code'] . ' </td>
                        <td> ' . $faculty1['faculty_experience'] . ' </td>
                        <td> ' . $faculty1['phone_no'] . ' </td>
        ...
        if (isset($_SESSION['faculty_information_print'])) {
            unset($_SESSION['faculty_information_print']);
        }
        $_SESSION['faculty_information_print'] = $table;
        return $table;
    }
   public function actionValuationfacultyreportPdf()
    {
        require(Yii::getAlias('@webroot/includes/use_institute_info.php'));
        $content = $_SESSION['faculty_information_print'];
        //print_r($content);exit;
        
        $pdf = new Pdf([
            'mode' => Pdf::MODE_CORE,
            'filename' =>  " Faculty Information .pdf",
            'format' => Pdf::FORMAT_A4,
            'orientation' => Pdf::ORIENT_PORTRAIT,
            'content' => $content,
            'cssFile' => '@vendor/kartik-v/yii2-mpdf/assets/kv-mpdf-bootstrap.min.css',
            'cssInline' => ' @media all{
                        table{border-collapse: collapse;  text-align: 
                            center;  font-family:"Roboto, sans-serif"; width:100%; font-size:12.5px; } 
                ...
                ',
            'options' => ['title' =>  " Faculty Information"],
            'methods' => [
                'SetHeader' => ["OFFICE OF THE CONTROLLER OF EXAMINATIONS ".$org_name],
                'SetFooter' => [ " Faculty Information " . ' PAGE :{PAGENO}'],
            ]
        ]);
        Yii::$app->response->format = \yii\web\Response::FORMAT_RAW;
        $headers = Yii::$app->response->headers;
    ...
    public function actionExcelValuationfacultyreport()
    {
