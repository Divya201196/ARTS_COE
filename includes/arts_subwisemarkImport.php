       ...
       // print_r($regulation);exit;
        ...
        else
        {
            ...
            $check_cia_marks_master = MarkEntryMaster::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'mark_type'=>$mark_type,'term'=>$term,'year'=>$year,'month'=>$month])->one();
             ...
             //$check_cia_marks_master_1 = MarkEntryMaster::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id])->orderBy('coe_mark_entry_master_id desc')->one();


              $check_cia_marks_master_1 = Yii::$app->db->createCommand('SELECT CIA  FROM coe_mark_entry_master WHERE student_map_id ="'.$student_map_id.'" and  subject_map_id ="'.$subject_map_id.'"  order by coe_mark_entry_master_id desc ')->queryOne();
             

                
           
           //print_r($check_cia_marks_master_1);exit;  
            ...
            $check_cia_marks_1 = MarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'category_type_id'=>$internal_id,'mark_type'=>$mark_type,'term'=>$term,'year'=>$year,'month'=>$month])->one();
            

            if(!empty($check_cia_marks_1) && $batchMapping->coe_batch_id==14 || $batchMapping->coe_batch_id==10 && $mark_type==28 )
            {
                 $cia_marks = $check_cia_marks_1['category_type_id_marks'];
                 $marks_available = 1;

                 //print_r($cia_marks);exit;


            }

            else if($batchMapping->coe_batch_id==14 || $batchMapping->coe_batch_id==10 && $mark_type==28)
            {
              $cia_marks = $check_cia_marks_master_1['CIA'];
             //print_r($cia_marks);exit;
               $marks_available = 1;
            }
        else
