...
{    
    ...
    {      
        ...
        else
        {
          ...
          //print_r($get_sub_info);exit;

            if($batchMapping->coe_batch_id=14 && $mark_type=28 )
            {
               $check_cia_marks = MarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'category_type_id'=>$internal_id,'mark_type'=>$mark_type,'term'=>$term,'year'=>$year,'month'=>$month])->one();
