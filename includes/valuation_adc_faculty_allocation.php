...
{    
    ...
    //$valuation_date1 = isset($line['G'])?date("Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($line['G'])):""; 
   // print_r($valuation_date);exit;
   ...
   //print_r($year);exit;
    ...
    {      
        ...
        $update_id = Yii::$app->db->createCommand("select val_faculty_all_id from coe_valuation_adc_faculty_allocate where exam_year = '".$year."' AND exam_month='".$month."' AND subject_code='".$qpcode."' AND subject_pack_i='".$coverno."'")->queryScalar();
    //echo ("select val_faculty_all_id from coe_valuation_adc_faculty_allocate where exam_year = '".$year."' AND exam_month='".$month."' AND subject_code='".$qpcode."' AND subject_pack_i='".$coverno."'");exit;
        ...
        {    
             $check_val_fact = Yii::$app->db->createCommand("SELECT sum(total_answer_scripts) FROM coe_valuation_adc_faculty_allocate  WHERE exam_month='" . $month . "' AND exam_year='" . $year . "' AND valuation_date='".date('Y-m-d',strtotime($valuation_date))."' AND valuation_session='".$valuation_session."' AND coe_val_faculty_id='".$check."'")->queryScalar(); 

            $total_pack1 = Yii::$app->db->createCommand("SELECT count(stu_reg_no) FROM coe_add_answerpack_regno  WHERE exam_month='" . $month . "' AND exam_year='" . $year . "'AND exam_type=27  AND answer_packet_number='" . $coverno . "'")->queryScalar(); 
            ...
            if($totpack!=0)
            {
                if(($ct)<=60)
                {
                    ...
                    if($scrutiny_id)
                    {
                        $insertqry='INSERT into coe_valuation_adc_faculty_allocate(scrutiny_status,coe_scrutiny_id,scrutiny_date,scrutiny_session,adc_board ,coe_val_faculty_id,exam_year,exam_month,subject_code,subject_pack_i,total_answer_scripts,valuation_date,valuation_session,valuation_status,created_at,created_by) values(1,"'.$scrutiny_id.'","'.$valuation_date.'","'.$valuation_session.'","'.$board.'","'.$check.'","'.$year.'","'.$month.'","'.$qpcode.'","'.$coverno.'","'.$totpack.'","'.date('Y-m-d',strtotime($valuation_date)).'","'.$valuation_session.'",3,"'.date('Y-m-d H:i:s').'","'.$userid.'") '; 
        ...
        {
            $getallocatedata = Yii::$app->db->createCommand("select * from coe_valuation_adc_faculty_allocate where exam_year = '".$year."' AND exam_month='".$month."' AND subject_code='".$qpcode."' AND subject_pack_i='".$coverno."'")->queryone();
            ...
            if(!empty($getallocatedata))
            {
                ...
                if($checkmarkentry==0)
                {
                    ...
                    $total_pack1 = Yii::$app->db->createCommand("SELECT count(stu_reg_no) FROM coe_add_answerpack_regno  WHERE exam_month='" . $month . "' AND exam_year='" . $year . "' AND answer_packet_number='" . $coverno . "'")->queryScalar();
                    ...
                    if($totpack!=0)
                    {
                        ...
                        {
                            $scrutiny_id = Yii::$app->db->createCommand("SELECT coe_scrutiny_id FROM coe_valuation_scrutiny  WHERE department='" . $board . "'")->queryScalar();
                             //print_r($scrutiny_id);exit;

                            if($scrutiny_id)
                            {
                            
                                $updateqry = 'UPDATE coe_valuation_adc_faculty_allocate SET coe_scrutiny_id="'.$scrutiny_id.'", scrutiny_date="'.date('Y-m-d',strtotime($valuation_date)).'", scrutiny_session="'.$valuation_session.'", adc_board="'. $scrutiny_id.'", coe_val_faculty_id="'.$check.'", valuation_date="'.date('Y-m-d',strtotime($valuation_date)).'", valuation_session="'.$valuation_session.'", updated_at="'.date('Y-m-d H:i:s').'", updated_by="'.$userid.'" WHERE val_faculty_all_id="'.$update_id.'"';
