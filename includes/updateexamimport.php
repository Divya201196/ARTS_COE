    ...
    { 
        ...
        if(!empty($sub_det))
        {
            ...
            $exam->select("coe_exam_timetable_id")
                ...
                ->where(['subject_mapping_id'=>$sub_det['coe_subjects_mapping_id']]);
            $exam_det = $exam->createCommand()->queryOne();
   //print_r($exam_det);exit;
            if(!empty($exam_det))
            {
                ...
                $qp_code = $line['L'];
                $cover_number = $line['M'];
  //print_r($cover_number);exit;
               
                $command1 = Yii::$app->db->createCommand('UPDATE coe_exam_timetable SET exam_date="'.$exam_date.'", qp_code="'.$line['L'].'",cover_number="'.$line['M'].'",updated_by="'.$created_by.'",updated_at="'.$created_at.'" WHERE coe_exam_timetable_id="'.$exam_det['coe_exam_timetable_id'].'" ')->execute();
