   ...
   public function createColumnsArray($end_column, $first_letters = '')
     ...
     public function getAttempts($stu_map_id,$exam_year,$exam_month)
     {
      ...
      public function getPartDetails($register_number,$part_no)
      {
        ...
        $total_add_credits_earn_cum_1 = 'SELECT sum(credit_points) as credits FROM coe_value_mark_entry as A JOIN sub as B ON B.coe_sub_mapping_id=A.subject_map_id JOIN coe_student_mapping as C ON C.coe_student_mapping_id=A.student_map_id JOIN coe_value_subjects as D ON D.coe_val_sub_id=B.val_subject_id  WHERE student_map_id="'.$getStu['stu_map_id'].'" and result like "%pass%" and part_no='.$part_no.' and b.course_type_id=122 ';
         $part_adds_1=Yii::$app->db->createCommand($total_add_credits_earn_cum_1)->queryScalar();
        

        if(!empty($part_adds_1))
         {

          $part_adds=$part_adds+$part_adds_1;
        ...
        //print_r($part_total_credits_send);exit;
